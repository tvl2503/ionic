...
// Sử dụng component IonIcon, IonText của ionic
  ...
  return (
    <div className="product--detail__content">
      ...
      <IonItemComponent className="category">
        <p>
          ...
          <span style={{ marginLeft: 8 }}>
            <Link
              to={`/products/${product?.category?.path}`}
              style={{ color: "#de0c19" }}
            >
              {" "}
      ...
      <IonItemComponent className="description">
        <div className="description__title">Mô tả:</div>
        <div className="description__content">{product.description}</div>
      </IonItemComponent>
      <IonText className="price">
        <p>
          {numberWithVND((product.price * (100 - product.percentReduce)) / 100)}
        </p>
        {product.percentReduce > 0 && (
          <div className="price__old">{numberWithVND(product.price)}</div>
        )}
      </IonText>
      ...
      <Button type="submit" onclick={handleClick}>
        <i className="fal fa-cart-plus"></i> Thêm vào giỏ hàng
      </Button>
    </div>
  );
};
